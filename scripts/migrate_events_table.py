...
async def migrate_events_table(db_url: str) -> None:
    ...
    engine = create_async_engine(
        db_url, echo=True, connect_args={"statement_cache_size": 0}
    )
    ...
    try:
        async with async_session() as session:
            ...
            await session.execute(check_and_add_column)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(
            "Usage: python migrate_events_table.py <postgresql_connection_url>"
        )
        print("\nExample:")
        print(
            "  python migrate_events_table.py 'postgresql://user:pass@host:port/db'"
        )
        sys.exit(1)
